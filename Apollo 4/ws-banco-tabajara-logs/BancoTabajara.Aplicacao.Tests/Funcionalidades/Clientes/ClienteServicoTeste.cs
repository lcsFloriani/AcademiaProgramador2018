...
namespace BancoTabajara.Aplicacao.Tests.Funcionalidades.Clientes
{
    ...
    public class ClienteServicoTeste
    {
        ...
        public void Clientes_Aplicacao_Deve_adicionar_cliente()
        {
            ...
            _repositorioFake.Setup(c => c.Adicionar(It.IsAny<Cliente>())).Returns(ObjetoMae.ObterClientePadraoComId);

            ClienteRegistraCommand command = ObjetoMae.ObterClienteRegistraCommand();

            //Ação
            long idRecebido = _servico.Adicionar(command);

            //Verifica
            idEsperado.Should().Be(idRecebido);
            _repositorioFake.Verify(repository => repository.Adicionar(It.IsAny<Cliente>()));
        }
        ...
        public void Clientes_Aplicacao_Deve_atualizar_cliente_e_retornar_true()
        {
            ...
            ClienteAtualizaCommand command = ObjetoMae.ObterClienteAtualizaCommand();

            //Ação
            var resultado = _servico.Atualizar(command);

            //Verifica
            resultado.Should().BeTrue();
            _repositorioFake.Verify(repository => repository.Atualizar(It.IsAny<Cliente>()));
        }
        ...
        public void Clientes_Aplicacao_Deve_retornar_cliente_por_id()
        {
            ...
            //Ação
            var resultado = _servico.BuscarPorId(idPesquisa);
        ...
        public void Clientes_Aplicacao_Deve_excluir_cliente_e_retornar_true()
        {
            ...
            _repositorioFake.Setup(c => c.Excluir(It.IsAny<Cliente>())).Returns(retorno);

            ClienteDeletaCommand command = new ClienteDeletaCommand();

            //Ação
            var resultado = _servico.Excluir(command);

            //Verifica
            resultado.Should().BeTrue();
            _repositorioFake.Verify(c => c.Excluir(It.IsAny<Cliente>()));
        }
        ...
        public void Clientes_Aplicacao_Deve_retornar_todos_os_clientes()
        {
            ...
            ClienteQuery query = ObjetoMae.ObterClienteQuery(quantidade);

            //Ação
            var clientes = _servico.Listagem(query);
