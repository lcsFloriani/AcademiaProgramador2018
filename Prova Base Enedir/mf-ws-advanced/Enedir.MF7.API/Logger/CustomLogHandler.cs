        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            return await base.SendAsync(request, cancellationToken)
                .ContinueWith(task =>
                {
                    ...
                    if (response.Content != null && response.Content is ObjectContent<PayLoadException>)
                    {
                        //Se tiver uma exception aqui ele atribui a mensagem do ExceptionPayLoad
                        logMetadata.ResponseExceptionPayLoad = (response.Content as ObjectContent<PayLoadException>).Value as PayLoadException;
                    }
        ...
        private void WriteEndLog(LogMetadata logMetadata)
        {
            ...
            if (logMetadata.ResponseExceptionPayLoad != null)
            {
                TraceLogManager.Error("[{0}] - Exception - Status: {1} - Message: {2}\r\nStackTrace: {3}",
                                                logMetadata.RequestMethod, logMetadata.ResponseExceptionPayLoad.ErrorCode,
